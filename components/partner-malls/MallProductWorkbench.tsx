...
export default function MallProductWorkbench({ value, onChange, logoUrl }: {
  ...
  return <section ref={sectionRef} className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_272px]">
    ...
    <aside className="min-w-0 rounded-xl border border-gray-200 bg-white p-4 lg:sticky lg:top-0">
      ...
      {value.length > 0 && <p className="mt-2 text-xs leading-5 text-gray-500">상품별 판매가를 입력한 뒤 저장하세요.<br />등록 후에도 가격을 수정할 수 있습니다.</p>}
      ...
      <div className="mt-3 space-y-3">{value.map(item => <div key={item.key} className="rounded-lg bg-gray-50 p-3">
        ...
        <div className="min-w-0 flex-1"><p className="break-words text-sm font-medium leading-5">{item.display_name}</p><p className="mt-1 text-xs leading-5 text-gray-500">{Object.keys(item.canvas_state).length}개 면 · {item.price === null ? '판매가 미입력' : `${item.price.toLocaleString()}원`}</p>{(item.color_name || item.color_hex) && <p className="text-xs text-gray-500">{item.color_name || item.color_hex}</p>}</div></div>
        <label className="mt-3 block text-xs font-medium text-gray-600">판매가 (원)<input aria-label={`${item.display_name} 판매가`} type="number" min="0" step="1" value={item.price ?? ''} placeholder="판매가 입력" onChange={e => onChange(value.map(v => v.key === item.key ? { ...v, price: e.target.value === '' ? null : Number(e.target.value) } : v))} className={`mt-1.5 w-full rounded-lg border bg-white px-3 py-2 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${item.price === null ? 'border-amber-300' : 'border-gray-200'}`} /></label>
