 ...
 * canary mall, never an order, and always leaves it inactive. No DELETE requests. */
...
(async()=>{
 ...
 const slug='e2e-design-'+Date.now();let mallId;let cleanup=false;const passed=[];
 ...
 try {
  ...
  assert(await c.getByText('25,800원',{exact:true}).count());await c.screenshot({path:path.join(out,'production-customer.png')});
  ...
  assert.equal(cart.state.items.length,1);const item=cart.state.items[0];assert.equal(item.quantity,2);assert.equal(item.pricePerItem,25800);assert.equal(item.partnerMallId,mallId);assert.equal(Object.keys(item.canvasState).length,4);
  ...
  fs.writeFileSync(path.join(out,'production-result.json'),JSON.stringify({passed,mallId,slug,inactive:cleanup,pageErrors:errors,completedAt:new Date().toISOString()},null,2));
  await browser.close();if(mallId&&!cleanup)throw new Error('Canary mall deactivation failed');
 }
 console.log(JSON.stringify({passed,mallId,inactive:cleanup,pageErrors:errors}));
