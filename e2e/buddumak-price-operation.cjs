...
(async()=>{
 ...
 try{
  ...
  const original=await read();const requestedIds=['343bcaa9-56ce-4014-a50e-c4046a0b6f17','673dccaa-f801-4166-8d6f-2cdaa135aec6'];assert(original.length>0&&original.length<=2);assert(original.every(v=>requestedIds.includes(v.id)&&v.display_name.includes('할머니의 부뚜막')));
  ...
  assert.deepEqual(errors,[]);fs.writeFileSync(out+'/buddumak-price-result.json',JSON.stringify({mallId,updated:after.map(v=>({id:v.id,price:v.price})),designUnchanged:true,checkoutUnitPrice:11900,quantity:2,pageErrors:errors,completedAt:new Date().toISOString()},null,2));console.log(`Buddumak ${after.length} current products priced at 11900, designs unchanged, customer checkout verified`);
