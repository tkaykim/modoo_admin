...
test('supplier invoice and recovery render without treating recovery as customer revenue',async()=>{
  ...
  vm.runInNewContext(compiled,{exports,require:(name:string)=>{
    ...
    if(name==='@/lib/supabase')return{createClient:async()=>({auth:{getUser:async()=>({data:{user:{id:'synthetic'}}})},from:(table:string)=>{
      let columns='';
      const q={select:(s:string)=>{columns=s;return q;},eq:()=>q,order:()=>q,limit:()=>q,single:async()=>({data:{role:'super_admin'}}),then:(resolve:(r:unknown)=>unknown)=>resolve({data:table==='legacy_order_cases'?cases:table==='legacy_order_cash_allocations'&&!columns.includes('cost_bank_transactions')?allocations:[],error:null,count:0})};
      return q;
    }})};
  ...
  assert.match(html,/>300원<\/td>/);
  assert.doesNotMatch(html,/>570원<\/td>|>370원<\/td>/);
});
