 ...
 *
 * Kept byte-compatible with modoo_app/lib/imageAlphaTrim.ts so the admin editor
 * produces the same alpha-tight artwork as the customer editor.
 *
 * Safety:
...
function loadImage(src: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => reject(new Error('[ALPHA-TRIM] image load failed'));
    img.src = src;
  });
}
...
export async function trimToAlphaBounds(
  ...
  try {
    ...
    console.warn('[ALPHA-TRIM] alpha trim scan failed, returning original', e);
    return { dataUrl, width: ow, height: oh, trimmed: false, originalWidth: ow, originalHeight: oh };
  }
  ...
  try {
    ...
    console.warn('[ALPHA-TRIM] getImageData failed (taint?), returning original', e);
    return { dataUrl, width: ow, height: oh, trimmed: false, originalWidth: ow, originalHeight: oh };
  }
  ...
  try {
    ...
    console.warn('[ALPHA-TRIM] alpha trim crop failed, returning original', e);
    return { dataUrl, width: ow, height: oh, trimmed: false, originalWidth: ow, originalHeight: oh };
  }
