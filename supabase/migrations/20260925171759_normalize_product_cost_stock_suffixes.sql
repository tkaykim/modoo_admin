-- Some legacy product options append a plain stock marker after a size note,
-- for example "110 (아동용) 품절". Strip that suffix before the annotation.
...
AS $function$
  WITH input AS (
    SELECT upper(btrim(regexp_replace(
      regexp_replace(COALESCE(p_size, ''), '\s*품절\s*$', '', 'i'),
      '\s*(\([^)]*\)|\[[^]]*\])\s*$',
      '',
      'g'
    ))) AS normalized_size
