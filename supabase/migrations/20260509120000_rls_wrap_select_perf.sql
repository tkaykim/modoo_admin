-- 성능 개선: RLS 정책의 함수 호출을 (SELECT ...)로 래핑해 쿼리당 1회 평가로 강제
-- 기존 20260502120000 정책과 논리 동일, per-row 재평가만 제거 (Postgres planner가 STABLE 함수 결과를 InitPlan으로 캐싱)
-- 함수 정의 자체는 변경하지 않음 (이전 마이그레이션의 정의를 그대로 사용)

-- profiles: JWT 클레임 대신 테이블 기반 (기존 JWT admin 은 거의 채워지지 않음)
DROP POLICY IF EXISTS "Admins can view all profiles" ON public.profiles;
CREATE POLICY "Admins can view all profiles" ON public.profiles
  FOR SELECT
  USING ((auth.uid() = id) OR (SELECT public.app_is_admin_or_super_admin()));

-- products
DROP POLICY IF EXISTS "Admins can manage all products" ON public.products;
CREATE POLICY "Admins can manage all products" ON public.products
  FOR ALL
  USING ((SELECT public.app_is_admin_or_super_admin()))
  WITH CHECK ((SELECT public.app_is_admin_or_super_admin()));
...
CREATE POLICY "Admins can manage announcements" ON public.announcements FOR ALL
  USING ((SELECT public.app_is_admin_or_super_admin())) WITH CHECK ((SELECT public.app_is_admin_or_super_admin()));
...
CREATE POLICY "Admins can delete faqs" ON public.faqs FOR DELETE USING ((SELECT public.app_is_admin_or_super_admin()));

DROP POLICY IF EXISTS "Admins can insert faqs" ON public.faqs;
CREATE POLICY "Admins can insert faqs" ON public.faqs FOR INSERT WITH CHECK ((SELECT public.app_is_admin_or_super_admin()));
...
CREATE POLICY "Admins can view all faqs" ON public.faqs FOR SELECT USING ((SELECT public.app_is_admin_or_super_admin()));
...
CREATE POLICY "Users can update their own pending inquiries" ON public.inquiries FOR UPDATE
  USING (
    ...
    OR (SELECT public.app_is_admin_or_super_admin())
  );
...
CREATE POLICY "Admins can update their own replies" ON public.inquiry_replies FOR UPDATE
  USING ((admin_id = auth.uid()) AND (SELECT public.app_is_admin_or_super_admin()));
