...
export async function GET(request: Request) {
  try {
    ...
    const message = error instanceof Error ? error.message : '고객가 단가 조회에 실패했습니다.';
...
export async function POST(request: Request) {
  try {
    ...
    // Strip factory_id concept — this table is global; only print_method_id/size matter.
    const adminClient = createAdminClient();
    const { data, error } = await adminClient
      .from('customer_print_method_pricing')
      .insert({
        print_method_id: validated.print_method_id,
        size: validated.size,
        max_width_cm: validated.max_width_cm,
        max_height_cm: validated.max_height_cm,
        pricing_model: validated.pricing_model,
        unit_price: validated.unit_price,
        base_price: validated.base_price,
        base_quantity: validated.base_quantity,
        additional_price_per_piece: validated.additional_price_per_piece,
    ...
    const message = error instanceof Error ? error.message : '고객가 단가 생성에 실패했습니다.';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    const payload = await request.json().catch(() => null);
    const id = payload?.id;
    if (typeof id !== 'string' || !id) return NextResponse.json({ error: 'id가 필요합니다.' }, { status: 400 });
    const validated = validatePricingRow(payload);
    if ('error' in validated) return NextResponse.json({ error: validated.error }, { status: 400 });
    ...
    const message = error instanceof Error ? error.message : '고객가 단가 수정에 실패했습니다.';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    const message = error instanceof Error ? error.message : '고객가 단가 삭제에 실패했습니다.';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
