...
export async function GET() {
  ...
  try {
    ...
    const [products, orders, shipments, settlements, qnas, runs, localProducts, designSessions] = await Promise.all([
      ...
      admin.from('naver_design_sessions').select('naver_order_id,status,job_count,submitted_job_count,submitted_at,updated_at').order('updated_at', { ascending: false }).limit(500),
    ]);
    for (const result of [products, orders, shipments, settlements, qnas, runs, localProducts]) if (result.error) throw result.error;
    // 디자인 접수 테이블이 아직 배포되지 않은 환경에서도 기존 네이버 운영 화면은 유지한다.
    if (designSessions.error) console.error('Naver design sessions unavailable:', designSessions.error);
    const designByOrder = new Map((designSessions.data || []).map((row) => [row.naver_order_id, row]));
    ...
    return NextResponse.json({
      data: {
        ...
        orders: (orders.data || []).map((row) => ({
          ...row,
          design_intake: designByOrder.get(row.naver_order_id) || null,
        })),
